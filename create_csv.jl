...
df.exposure = exposure.(
    Anniversary(Year(1)),   # The basis for our exposures
    df.issue_date,                             # The `from` date
    df.term_date,
    df.status .== "Surrender";                              # the `to` date array we created above
    study_start = Date(2006,6,15),
    study_end = Date(2020,2,29),
    left_partials = false,
    right_partials = true,
)
df_yearly = flatten(df,:exposure)
df_yearly.exposure_fraction = map(e -> yearfrac(e.from, e.to + Day(1), DayCounts.Thirty360()), df_yearly.exposure)
